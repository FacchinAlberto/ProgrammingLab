...
class CSVTimeSeriesFile():
    def __init__(self, name=None):
        if type(name) == str:
            ...
            raise ExamException('ERROR: il nome del file non è una stringa')
                                ...
                                if '-' not in elem[0]:
                                    ...
                                    if converted_in_int:
                                        ...
                                        try:
                                            ...
                                            print('ERROR: impossibile convertire valore paseggeri "{}" della riga {} in int'.format(elem[1], current_line))
...
def year_in_file(time_series, year):  
    ...
    for item in time_series:
        ...
        list_of_years.append(int(year_and_month[0]))    #da verificare int()
                    ...
                    if first_year == last_year:
                        ...
                        for item in time_series:
                            ...
                            if int(year_and_month[0]) >= first_year and int(year_and_month[0])<= last_year:
                                ...
                                #if len(my_dict[year_and_month[1]]) == 0:
                                 #   my_dict[year_and_month[1]] = []
                                #my_dict[year_and_month[1]].append(int(item[1]))
                        ...
                        for i in range (0,12):
                            ...
                            for j in range(0, len(months)):
                                if i+1 == months[j]:
                                    if prev_value == 0:
                                        prev_value = passengers[j]
                                    else:
                                        avg_variation[i] += passengers[j] - prev_value
                                        prev_value = passengers[j]
                            if diff == 1:
                                if freq_of_month[i] < 2:
                                    avg_variation[i] = 0
                            elif diff > 1 and freq_of_month[i] < 2:
                                avg_variation[i] = 0
                            elif diff > 1 and freq_of_month[i] <= diff:
                                avg_variation[i] /= diff-1
                            else:
                        ...
                        for i in range(0, 12):
                            for j in range(0, diff):
                                #dovrei scrivere diff-1, ma visto che per es. 2021-2019 = 2, ed io devo considerare 3 anni (2019, 2020, 2021) poosso usare diff
                                avg_variation[i] += passengers[(12*(j+1))+i]-passengers[(12*j)+i]   
                                #esempio della prima iterazione del ciclo "esterno" (utilizzando 1949, 1950, 1951):
                            ...
                            avg_variation[i] /= diff
                            #divido ogni elemento di avg_variation per il "numero di sottrazioni fatte"
                        """
